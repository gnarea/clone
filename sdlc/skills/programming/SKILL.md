...
- Files SHOULD NOT span more than 800 LOC, including comments and new lines. 1 kLOC or more MUST be treated as a defect to fix.
...
- Inequalities SHOULD be ordered, with `<`/`<=` preferred over `>`/`>=` so that operands read from lower to upper bound (e.g., `if (MAX_TTL_SECONDS < ttl)`).
...
- Constants, variables, fields, and the like, SHOULD be named after what they hold, rather than what they are used for, and SHOULD match the taxonomy of the user and programming interfaces.
...
- A rename that only improves a name MUST NOT be made where it would break a published surface.
- The user-facing vocabulary MAY differ from the code's, where the product hides detail that the code needs to be accurate about. Each MUST then be used consistently within its own domain, and the mapping MUST be documented.
...
- Where a sequence of side effects cannot be atomic, it MUST be ordered so that a retry after any partial failure converges on the intended state, and the ordering MUST carry a comment saying why.
- Input that can never succeed MUST be dropped and recorded, never retried.
...
- Where the platform asks the process to stop (e.g., SIGTERM), it MUST stop accepting work and exit within the grace period it's given.
...
- Where the input or the configuration is ambiguous or incomplete, the code MUST fail closed and refuse to proceed, rather than guess which of two intents was meant.
...
- Setup that only some code paths need MUST run when those paths run, rather than when the module loads. An implementation chosen at runtime, such as a storage or key-management adapter, MUST be loaded only once it's been chosen.
...
- Tests whose arrange, act, and assert (AAA) blocks overlap substantially MUST be generated dynamically using a data-driven approach (aka _parameterised testing_), as long as they test the same unit.
...
- **Private/internal** functions, procedures, and the like, SHOULD NOT be unit tested directly: test them through the public/exported counterparts that use them. Where visibility is widened for testability, the concession MUST be stated on the member.
- Tests SHOULD use real dependencies rather than test doubles, including databases and cryptographic operations. Where the real thing is used, each test run MUST get its own isolated instance or namespace.
...
- A suppression of an automated check MUST be at the narrowest scope the tool offers, placed at the line it excuses, and MUST state its reason. Where the reason is an upstream defect, it MUST link to it.
