...
A _prototype_ is code written to answer a question, and discarded once it has. Every rule in the skill applies, except as follows.
...
- A path outside the main journeys MAY be left unimplemented. Where it is, the code MUST fail immediately and say that the case is unsupported, so that it can't be mistaken for a defect.
